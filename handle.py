...
def transfer(dirPath,replace_dict):
    #transfer doc to docx
    word = wc.Dispatch("Word.Application")
    #i = 0
    for roots, dirs, files in os.walk(dirPath):
        for file in files:
            if file.endswith("doc") and file[0] != "~":
                fileRoot = os.path.join(roots, file)
                doc = word.Documents.Open(fileRoot)
                files_ = file.split('.')
                doc.SaveAs(current_path+"/生成文件/"+str(replace_dict.get("梃梃梃"))+str(replace_dict.get("拉-00000"))+files_[0]+".docx",12)
                #i = i+1
                doc.Close
                word.Quit
...
def kill_process():
    ...
    pro = 'taskkill /f /im %s'% process_name
    os.system(pro)

def transfer_(dirPath):
    # transfer docx to doc
    word = wc.Dispatch("Word.Application")
    i = 0
    for roots, dirs, files in os.walk(dirPath):
        for file in files:
            if file.endswith("docx") and file[0] != "~":
                ...
                doc.SaveAs("{}".format(fileRoot[:-1]), 0)
                i = i + 1
                doc.Close
                word.Quit
...
def main(replace_dict):
    ...
    os.remove(current_path+"/生成文件/质量记录.doc")
    os.remove(current_path + "/生成文件/设备表.doc")
    os.remove(current_path + "/生成文件/材料表.doc")
    os.remove(current_path + "/生成文件/目录.doc")



if __name__ == "__main__":

    filedir = r"E:\delivery_design_work\word_"
    transfer(filedir)
    #time.sleep(5)
    kill_process()
    main()
